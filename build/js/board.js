...
function setTasksHtml() {
    allTaskObjects.forEach((elem, index)=>{
        let card = /* HTML */ `<div class="task flex-column" draggable="true" data-storyType="${elem.storyType}" data-storyIndex="${index}">
            ...
            <div class="participants-and-urgency flex">
                <div class="participants flex">${renderParticipants(elem.participants)}</div>
...
function renderStory(elem, id) {
    ...
    draggableObjs = document.querySelectorAll(".task");
    shiftParticipants();
    checkForEmptyColumns();
    checkForFilledColumns();
    setDragDrop();
...
function checkForEmptyColumns() {
    if(document.querySelector('#toDo').querySelectorAll('.task').length === 0) {
        document.querySelector('.no-task-to-do').classList.remove('disNone');
    }
    if(document.querySelector('#inProgress').querySelectorAll('.task').length === 0) {
        document.querySelector('.no-task-in-progress').classList.remove('disNone');
    }
    if(document.querySelector('#awaitFeedback').querySelectorAll('.task').length === 0) {
        document.querySelector('.no-feedback-awaited').classList.remove('disNone');
    }
    if(document.querySelector('#done').querySelectorAll('.task').length === 0) {
