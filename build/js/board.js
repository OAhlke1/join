...
function getParticipantsHtml(index) {
    ...
    for(let i=0; i<allParticipantsKeys[index].length; i++) {
        if(!allTaskObjects[index].participants[allParticipantsKeys[index][i]].lastName) {
            pList += `<div class="participant flex-center" style="background-color: ${allTaskObjects[index].participants[allParticipantsKeys[index]]}"><p class="initials">${allTaskObjects[index].participants[allParticipantsKeys[index][i]].sureName[0]}</p></div>`;
        }else if(!allTaskObjects[index].participants[allParticipantsKeys[index][i]].sureName) {
            pList += `<div class="participant flex-center" style="background-color: ${allTaskObjects[index].participants[allParticipantsKeys[index]]}"><p class="initials">${allTaskObjects[index].participants[allParticipantsKeys[index][i]].lastName[0]}</p></div>`;
        }else {
            pList += `<div class="participant flex-center" style="background-color: ${allTaskObjects[index].participants[allParticipantsKeys[index]]}"><p class="initials">${allTaskObjects[index].participants[allParticipantsKeys[index][i]].sureName[0]}${allTaskObjects[index].participants[allParticipantsKeys[index][i]].lastName[0]}</p></div>`;
        }
    }
    return pList;
}
...
function reGetParticipantsHtml(index) {
    ...
    for(let i=0; i<participantsArrayOverlay.length; i++) {
        if(!participantsArrayOverlay[i].lastName) {
            pList += `<div class="participant flex-center" style="background-color: ${participantsArrayOverlay[i].color}"><p class="initials">${participantsArrayOverlay[i].sureName[0]}</p></div>`;
        }else if(!participantsArrayOverlay[i].sureName) {
            pList += `<div class="participant flex-center" style="background-color: ${participantsArrayOverlay[i].color}"><p class="initials">${participantsArrayOverlay[i].lastName[0]}</p></div>`;
        }else {
            pList += `<div class="participant flex-center" style="background-color: ${participantsArrayOverlay[i].color}"><p class="initials">${participantsArrayOverlay[i].sureName[0]}${participantsArrayOverlay[i].lastName[0]}</p></div>`;
        }
    }
    return pList;
}
...
function setTaskType() {
    ...
    resetTaskTypeOnFtp(+dragged.closest('.task').getAttribute('data-taskindex'));
}
...
function renderChosenListOverlay() {
    ...
    participantsArrayOverlay.forEach((elem, i)=>{
        document.querySelector('.overlay-card .chosen-list.back').innerHTML += /* HTML */ `<li><div class="flex flex-center circle" onclick="removeParticipantOverlay(${i})" style="background-color: ${elem.color}">
...
function reRenderChosenListFrontOverlay(index) {
    ...
    if(allTaskObjects[index].participants) {
        let elem = allTaskObjects[index].participants;
        for(let i=0; i<participantsArrayOverlay.length; i++) {
            list += /* HTML */ `<li class="flex flex-center"><div class="flex flex-center circle" onmouseover="showNameOverlay(${i})" onmouseleave="hideNameOverlay(${i})" style="background-color: ${participantsArrayOverlay[i].color}">
...
function taskIsStillOnRemote(elem, id) {
 ...
 */
async function closeOverlay(index) {
    if(!index) {
        index = +document.querySelector('.overlay-card').getAttribute('data-taskindex');
    }
    document.querySelector('.tasks-overlay').classList.add('disNone');
    checkIfTaskExistsOnRemote(index);
}

async function checkIfTaskExistsOnRemote(index) {
    let response = await fetch(tasksURL+'.json');
    response = await response.json();
    for(let i=0; i<response.length; i++) {
        if(response[i].taskId === allTaskObjects[index].taskId) {
