    ...
    for(let h=0; h<keyWords.length; h++) {
        for(let j=0; j<allTaskObjects[i].participants.length; j++) {
            for(let [key, value] of Object.entries(allTaskObjects[i].participants[j])) {
                if(searchBar.value.includes(keyWords[h])) { return true; }
            }
        }
    }
    return false;
...
function styleUrgencyOverlay(index) {
    if(allTaskObjects[allTaskKeys[index]].urgency === "low") {
        document.querySelectorAll('.overlay-card .choose-prio-button')[2].classList.add('prio-low-button-bg-color');
    }else if(allTaskObjects[allTaskKeys[index]].urgency === "medium") {
        document.querySelectorAll('.overlay-card .choose-prio-button')[1].classList.add('prio-medium-button-bg-color');
    }else if(allTaskObjects[allTaskKeys[index]].urgency === "high") {
        document.querySelectorAll('.overlay-card .choose-prio-button')[0].classList.add('prio-high-button-bg-color');
    }
}
...
function resetUrgency(event, index, prio) {
    if(allTaskObjects[allTaskKeys[index]].urgency === "low") {
        document.querySelectorAll('.overlay-card .choose-prio-button')[2].classList.remove('prio-low-button-bg-color');
    }else if(allTaskObjects[allTaskKeys[index]].urgency === "medium") {
        document.querySelectorAll('.overlay-card .choose-prio-button')[1].classList.remove('prio-medium-button-bg-color');
    }else if(allTaskObjects[allTaskKeys[index]].urgency === "high") {
        document.querySelectorAll('.overlay-card .choose-prio-button')[0].classList.remove('prio-high-button-bg-color');
    }
    event.target.closest('.choose-prio-button').classList.add(`prio-${prio}-button-bg-color`);
    newUrgency = prio;
...
function clearForm() {
    ...
    document.querySelectorAll('.add-task-overlay .triangle').forEach((elem)=>{elem.classList.remove('rotated')});
}
